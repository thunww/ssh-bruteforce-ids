...
def aggregate_window_features(
    ...
    # byte_ratio: fwd_bytes / (bwd_bytes + 1) — brute-force có ít bwd bytes
    fwd_b = pd.to_numeric(window_df.get("Total Length of Fwd Packet", pd.Series(dtype=float)), errors="coerce").fillna(0.0).sum()
    bwd_b = pd.to_numeric(window_df.get("Total Length of Bwd Packet", pd.Series(dtype=float)), errors="coerce").fillna(0.0).sum()
    row["byte_ratio"] = float(fwd_b / (bwd_b + 1.0))

    # syn_ack_ratio: SYN / (ACK + 1) — SYN probing pattern
    syn_s = pd.to_numeric(window_df.get("SYN Flag Count", pd.Series(dtype=float)), errors="coerce").fillna(0.0).sum()
    ack_s = pd.to_numeric(window_df.get("ACK Flag Count", pd.Series(dtype=float)), errors="coerce").fillna(0.0).sum()
    row["syn_ack_ratio"] = float(syn_s / (ack_s + 1.0))

    # packet_rate_per_window: (total fwd + bwd packets) / window_duration
    fwd_pkt = pd.to_numeric(window_df.get("Total Fwd Packet", pd.Series(dtype=float)), errors="coerce").fillna(0.0).sum()
    bwd_pkt = pd.to_numeric(window_df.get("Total Bwd packets", pd.Series(dtype=float)), errors="coerce").fillna(0.0).sum()
    win_dur = float((window_end - window_start).total_seconds())
    row["packet_rate_per_window"] = float((fwd_pkt + bwd_pkt) / max(win_dur, 1.0))
